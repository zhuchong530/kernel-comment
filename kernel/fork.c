					...
					int node)
{
	...
	/* CLONE_FS不能与CLONE_NEWNS或CLONE_NEWUSER同时设置 */
	 ...
	 */
	/* 防止参数init进程的兄弟进程，
	   只有init进程的signal->flags & SIGNAL_UNKILLABLE 为真
	   因为进程退出时实际上成为僵尸进程(zombie),而要通过init进程将其回收，而如果此进程为init的兄弟进程，则没有办法将其回收
	 */
	 ...
	 */
	/* 如果新的进程有新的pid或者用户空间，则不能让他共享父进程的线程组或者信号处理或者父进程 */
	if (clone_flags & CLONE_THREAD) {
	...
	/* futex初始化，用于SystemV IPC */
	/* futex(Fast Userspace Mutex) 它是一种由用户态和内核态共同完成的同步机制。
	   创建时是在用户空间通过mmap申请一片共享内存以便多进程间共同访问此futex，用户程序首先访问用户态的futex，
	   只在判断出存在冲突（如一个进程已经拥有此futex，另一个进程申请访问，此时便存在一个冲突）时才进入内核态进行仲裁同步。
	   用户空间的访问和冲突判断由glibc库完成，冲突仲裁由内核的futex模块完成。
	   内核仲裁：将用户态的锁置上等待标志表明有锁的等待者存在，并调用schedule()将锁申请者挂起；
	   当锁的拥有者释放锁(由glibc库完成)时，检查发现该锁有等待者就进入内核将等待者唤醒。
	...
	*/
...
#endif
	retval = -EAGAIN;
	/* 检查tsk->signal->rlim[RLIMIT_NPROC].rlim_cur是否小于等于用于拥有的进程数，rlim结构体表示相关资源的最大值 */
	if (atomic_read(&p->real_cred->user->processes) >=
			task_rlimit(p, RLIMIT_NPROC)) {
		/* INIT_USER(sched.h)是root权限， 检查父进程是否有root权限 */
	 ...
	 */
	retval = -EAGAIN;
	/* 进程数量是否超过系统允许最大进程数，最大进程数和内存有关，一般原则是所有的进程内核栈(默认8K)加起来不超过总内存的1/8
	   可以通过/proc/sys/kernel/threads-max改写
	 */
	if (nr_threads >= max_threads)
		goto bad_fork_cleanup_count;

	delayacct_tsk_init(p);	/* Must remain after dup_task_struct() */
	/* 清除PF_SUPERPRIV表示进程使用了超级用户权限 和 PF_WQ_WORKER(使用了工作队列) */
	p->flags &= ~(PF_SUPERPRIV | PF_WQ_WORKER | PF_IDLE);
	/* 设置PF_FORKNOEXEC表示此子进程还没有进程exec调用 */
	p->flags |= PF_FORKNOEXEC;
	INIT_LIST_HEAD(&p->children); /* 初始化子进程的子进程链表 */
	INIT_LIST_HEAD(&p->sibling);  /* 初始化子进程的兄弟链表 */
	...
	/* 判断是否设置CLONE_SIGHAND,如果是（线程必须为是），增加父进程的sighand引用计数
	   如果否（创建的必定是子进程），将父进程的sighand_struct复制到子进程中
	 */
	...
	/* 如果是进程： 则将父进程的mm_struct结构复制到子进程中， 然后修改当中属于子进程有别于父进程的信息（如页目录）
	   如果是线程： 则将子进程的mm指针和active_mm指针都指向父进程的mm指针所指结构
	 */
    ...
    /* TODO: 这是copy_thread的注释， 新版本的函数多了tls，需要更新
   ...
   */
	...
	/* 如果设置了CLONE_CHILD_SETTID,则将task_struct中的set_child_tid指向用户空间的child_tidptr,否则置空 */
	p->set_child_tid = (clone_flags & CLONE_CHILD_SETTID) ? child_tidptr : NULL;
	/*
	 * Clear TID on mm_release()?
	 */
    /* 如果设置了CLONE_CHILD_CLEARTID,则将task_struct中的clear_child_tid指向用户空间的child_tidptr,否则置空 */
	...
	/* 将子进程的PID设置为分配的PID在全局namespaces中分配的值，在不同的namespace中进程的PID不同
	   而p->pid保存的是全局的namespace中所分配的PID
	 */
	p->pid = pid_nr(pid);
	if (clone_flags & CLONE_THREAD) {
		/* 创建的是线程 */
		...
		p->group_leader = p;
		p->tgid = p->pid;		/* tgid与pid一致，所以当创建子线程时，tgid与主线程一致 */
	}

	p->nr_dirtied = 0;			/* 初始化页框中脏页数量为0 */
	/* 初始化脏页数量临界值，当脏页数量达到临界值是时，会调用balance_dirty_page()将脏页写入磁盘 */
	 ...
	 */
	/* 到此系统中已经存在此进程（线程），但是他还不能够执行，需要等待父进程对其处理，这里会上锁 */
	...
	*/
	...
	if (likely(p->pid)) {
		/* 如果子进程需要跟踪，就将current->parent 赋给 tsk->parent，并将子进程插入调试程序的跟踪列表中 */
		ptrace_init_task(p, (clone_flags & CLONE_PTRACE) || trace);

		/* p->pids[PIDTYPE_PID].pid = pid */
		init_task_pid(p, PIDTYPE_PID, pid);
		/* 如果是子进程（其实就是判断->exit_signal是否大于0），创建的是线程的话， exit_signal的值为-1 */
		if (thread_group_leader(p)) {
			...
			/* 创建的是线程， 这里的处理导致线程会共享信号 */
			current->signal->nr_threads++;
			atomic_inc(&current->signal->live);
			atomic_inc(&current->signal->sigcnt);
			/* 将新线程的thread_group节点加入到线程组的领头线程的thread_group链表中 */
			list_add_tail_rcu(&p->thread_group,
					  &p->group_leader->thread_group);
			/* 将新线程的thread_node节点加入到新线程的signal->thread_head中 */
			list_add_tail_rcu(&p->thread_node,
					  &p->signal->thread_head);
		}
		/* 将新进程描述符的pid结构 插入pid_hash */
